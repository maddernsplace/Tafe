...
export default function Courses() {
  ...
  const { courses, activeCourses, assessments, addCourse, updateCourse, deleteCourse } = useApp()
  ...
  const CourseCard = ({ course }) => {
    ...
    return (
      <div className="course-list-card" onClick={() => navigate(`/courses/${course.id}`)}>
    ...
    <div className="page">
      <div className="page-header">
        <div>
          ...
          <p className="page-sub">{activeCourses.length} active · {inactiveCourses.length} inactive</p>
        </div>
      ...
      {activeCourses.length === 0 && inactiveCourses.length === 0 ? (
        ...
        <>
          {activeCourses.length === 0 ? (
            <p className="text-muted" style={{ marginBottom: 16 }}>No active courses. Activate a course from the Inactive section below.</p>
          ) : (
          ...
          {inactiveCourses.length > 0 && (
            <div className="inactive-section">
              ...
              {showInactive && (
                <div className="courses-list inactive-list">
                  {inactiveCourses.map(course => <CourseCard key={course.id} course={course} />)}
                </div>
              )}
            </div>
          )}
